...
class Prestamos extends Controller
{
    ...
    public function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            if ($data[$i]['estado'] == 1) {
                ...
                $data[$i]['estado'] = '<span class="badge badge-primary">Calificado</span>';
                $data[$i]['acciones'] = '<div>
                <a class="btn btn-danger" target="_blank" href="'.base_url.'Prestamos/ticked/'. $data[$i]['id'].'"><i class="fa fa-file-pdf-o"></i></a>
                <div/>';
            }
    ...
    public function registrar()
    {
        ...
        if (empty($libro) || empty($estudiante) || empty($cantidad) || empty($fecha_prestamo) || empty($fecha_devolucion)) {
            ...
            if ($verificar_cant['cantidad'] >= $cantidad) {
                ...
                if ($data > 0) {
                    $msg = array('msg' => 'Alumno Registrado', 'icono' => 'success', 'id' => $data);
                } else if ($data == "existe") {
                    $msg = array('msg' => 'Intentos Maximos alcanzados', 'icono' => 'warning');
                } else {
    ...
    public function entregar($id)
    {
        ...
        if ($datos == "ok") {
            $msg = array('msg' => 'Alumno Calificado', 'icono' => 'success');
        }else{
            $msg = array('msg' => 'Error al Calificar', 'icono' => 'error');
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
